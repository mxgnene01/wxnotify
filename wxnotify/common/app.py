...
tornado_define("config_file", default="", type=str, help="path to config file", group='main')
...
class WebApplication(tornado.web.Application, SingletonMixin):
    ...
    def __init__(self, settings=None):
        ...
        route = import_object('wxnotify.common.route')
...
def setup_options():
    ...
    if os.path.exists(tornado_options.config_file):
        parse_config_file(tornado_options.config_file)

    if tornado.version_info < (3, 0, 0, 0):
        for key, option in tornado_options.iteritems():
            LOG.info('Options: (%s, %s)', key, option.value())
    else:
        for key, option in tornado_options.items():
            LOG.info('Options: (%s, %s)', key, option)
